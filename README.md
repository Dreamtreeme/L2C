...
완성된 것은 전체 범용 replay가 아니라, **고정 가능한 부모 경로를 pHash/OCR
서명으로 재사용하고 마지막 가변 판단은 LLM에게 남기는 경험 기반 탐색**입니다.

2026-08-12 깨끗한 커밋 `8c36be3`에서 원티드와 사람인을 각각
`자율 탐색 → Critic 승격 → 경험 기반 탐색`으로 한 번씩 비교했습니다.

| 사이트 | 자율 탐색 | 경험 기반 탐색 | 추론 | 토큰 | 완료 경로 |
|---|---:|---:|---:|---:|---:|
| 원티드 iOS 2건 | 61.36초 | 56.26초 | 10회 → 8회 | 65,279 → 55,439 | 2개 |
| 사람인 AI 2건 | 76.12초 | 74.07초 | 13회 → 10회 | 91,981 → 74,066 | 2개 |

네 실행 모두 요청한 2건 저장과 품질 게이트를 통과했습니다. 승격 비용을 포함한
손익분기점 추정은 원티드 2.83회, 사람인 0.96회입니다. 이 값은 사이트별 1개
짝 표본이므로 성공률이나 일반 성능으로 확대 해석하지 않습니다.

같은 FastAPI 수명에서 원티드 iOS와 백엔드 수집 요청을 연속 실행한 제품 회귀도
`2/2` 통과했습니다. 두 답변은 수집 문서 ID를 인용했고, OCR 작업자 PID는
`19472`로 유지됐으며 요청마다 자동화 브라우저 바인딩이 해제됐습니다.

현재 결론은 다음과 같습니다.

- pHash 기반 Reflex는 원티드와 사람인의 단일 짝 표본에서 reasoning 감소 효과를 확인했다.
