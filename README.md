...
[지휘자 — Gemini 3.5 Flash]
  ...
  도구 선택 및 순차 호출
    ...
    │
    └─ [RAG 검색 도구] (적재된 DB 조회로 충분할 때)
        SQL 1차 필터링
        Numpy Cosine Similarity 기반 의미 검색
        인용 ID 부여
    ↓
[지휘자 — Gemini 3.5 Flash]
  진행 상황 평가
...
- [x] Phase 3.5: OmniParser(Set-of-Marks) 로컬 파이프라인 실제 구현 및 순수 비전 본문 JSON 추출
  ...
  - [x] 3. `som_engine.py` 구현 (YOLOv8 + EasyOCR을 통한 요소 검출, 중복 제어 NMS, 마크 이미지 합성 및 좌표 매핑, 2.25초 처리 시간 확인)
...
- [x] Phase 7: 적재 데이터 활용 RAG 검색 도구 구축 및 지휘자 통합
  - [x] 1. RAG 인프라 구축 (`database.py`, `query_cache.py`)
    - [x] 768차원 `embedding` BLOB 컬럼 추가 및 동적 마이그레이션
    - [x] `text-embedding-004` 임베딩 결과 LRU 캐시 (`EmbeddingLRUCache`)
  - [x] 2. RAG 검색 엔진 구현 (`rag_engine.py`)
    - [x] Gemini 3.5 Flash 기반 구조화 필터 추출 및 `days_limit` 역산 날짜 연산을 통한 SQL 1차 필터링
    - [x] Numpy 단일 루프 Cosine Similarity 연산
    - [x] Top-1과 Top-2~5의 평균 유사도 격차로 검색 결과 변별력 검증 및 미달 시 LLM 호출 우회
  - [x] 3. 지휘자 도구 통합 및 라우팅 (`nodes.py`)
    - [x] RAG 검색 도구를 지휘자의 function calling 도구로 등록
    - [x] 기존 비전 에이전트 수집 흐름을 지휘자의 보조 도구로 통합 및 순차 호출 라우팅
  - [x] 4. 답변 생성 및 인용 검증 (`nodes.py`)
    - [x] XML 구조화 컨텍스트 주입, 스트리밍 응답, 온도 0.0 설정
    - [x] 답변 생성 후 인용 ID 정합성 검사 및 위반 ID는 `[출처 확인 불가]` 마커로 치환 (`validate_citations`)
  - [x] 5. E2E RAG 동작 및 미적재 정보 거절 테스트 검증 (`test_rag_qa.py`)
    - [x] 미적재 정보 질의 시 유사도 격차 미달로 즉시 거절 응답을 반환하는 케이스 검증
...
```
L2C/
├── classic/          전통 자동화 (베이스라인 + Phase 8 자동 생성 스크립트 적재 대상)
│   ├── automation/     Playwright DOM 파싱
│   ├── extractor/      VLM 정형화
│   └── README.md
│
├── agent/            비전 LLM 에이전트
│   ├── tools/          화면 인식·행동·추출·RAG 검색 도구
