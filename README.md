...
**비전 LLM 에이전트**(browser-use 등)는 DOM selector 없이 화면을 탐색할 수
있지만, 매 작업에서 같은 화면까지 다시 추론하면 시간과 API 비용이 반복해서
발생합니다.

L2C는 두 방식의 장점을 결합하는 구조를 실험합니다. 지원할 사이트에는 공식
주소와 화면 단서를 담은 작은 선언형 프로필을 등록하고, 검색과 상세 탐색은
사이트별 실행 코드가 아니라 공통 비전 작업자가 수행합니다. 자율탐색에서 성공한
행동 중 Critic과 화면 증거가 안전하다고 판단한 원자 단계만 Reflex Recipe로
승격합니다. 검색 진입·입력처럼 고정 가능한 부모 경로는 재생하고, 현재 공고
...
자율탐색보다 누적 추론 비용이 낮아지는지를 검증하는 것이 핵심입니다.
...
해석하지 않습니다. 현재는 Phase 8에서 자율탐색 기록을 반복탐색에 재사용해
...
2026-07-28 현재 원티드·사람인·잡코리아·고용24의 격리 DB 회귀에서
자율탐색과 반복탐색 모두 요청한 공고 저장 품질을 통과했습니다. 반복탐색
실행시간은 원티드 `81.78초 → 72.27초`, 사람인 `63.20초 → 47.51초`,
잡코리아 `76.12초 → 34.52초`로 줄었지만, 고용24는 검색 입력 한 단계만
재사용되어 `59.43초 → 64.04초`로 개선되지 않았습니다.
...
- 다음 단계는 사이트별 표본 수를 늘려 반복 성공률·잘못된 클릭률·공고당 비용을 비교하는 것이다.
...
| 신규 사이트 추가 | 사이트별 어댑터 코드 작성 | 매 화면 LLM 탐색 | DOM 코드 없이 프로필과 첫 비전 탐색으로 시작 |
| 반복 작업 비용 | 낮음 | 같은 화면도 LLM 비용 반복 | 승격된 단계만 추론 생략, 가변 단계 비용은 유지 |
| UI 변경 대응 | selector 수정 필요 | 현재 화면에서 다시 추론 | 서명 불일치 시 재생 중단 후 비전 탐색으로 폴백 |
| 비용 판단 | 실행당 비용이 낮고 일정 | 실행당 비용이 높고 일정 | Critic 비용을 포함한 반복 손익분기점으로 평가 |
...
반복탐색에도 가변 카드와 상세 화면의 LLM 호출이 남아 있으므로 Reflex를 최종
...
- [ ] **Phase 8: 피드백 루프 기반 Reflex Recipe 승격 (현재 단계)**
  ...
  > 현재 진행 중인 핵심 단계. 비전 에이전트가 처음부터 정답 스크립트를 만들도록 제약하지 않고, 탐색 중 생성한 행동 제안과 실제 실행 결과를 관찰한다. 현재 구현은 성공한 한 실행의 원자 단계 중 Critic과 ROI 증거가 승인한 단계만 활성 Reflex Recipe로 저장한다. Reflex는 해당 단계의 LLM 추론을 생략하되, 검색어·수집 개수·사이트 범위처럼 바뀌는 값은 지휘자가 파라미터로 주입한다. 실패하거나 확신이 낮은 상황에서는 다시 비전 탐색으로 폴백한다. 여러 실행의 반복 성공을 승격 전 필수 조건으로 두는 정책과 negative example 학습은 아직 완료되지 않았다.
