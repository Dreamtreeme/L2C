...
> 검증된 행동 경로를 화면 상태 조건에 따라 재사용하고, 검증 실패 시 LLM 자율 탐색으로 복귀하는 LangGraph 기반 Windows Computer-Use Agent

**프로젝트** · 개인 개발 · Windows 로컬 End-to-End AI Agent

**기여 범위** · 문제 정의 · Agent/Backend 아키텍처 · 구현 · 평가 설계와 실행

**핵심 영역** · AI Agent Engineering · Computer Use · Evaluation · Backend

`Python` `FastAPI` `LangGraph` `Stateful Workflow` `Structured Output` `Guardrails`
`Safe Fallback` `Observability` `Agent Evaluation` `Regression Testing` `OCR`

[데모 가이드](./docs/product_demo.md) · [아키텍처](./ARCHITECTURE.md) ·
[Benchmark & Evidence](./docs/evidence/l2c_metrics_20260822.md) · [핵심 코드](#핵심-구현) ·
[테스트 구조](./agent/tests/README.md)

![L2C 채용 시장 조사 화면](./docs/assets/l2c-chat-ui.png)

## Engineering Highlights

- 조사 Agent와 Vision Worker를 두 LangGraph로 분리하고 FastAPI·SSE로 실행 상태와 구조화 결과를 전달합니다.
- URL 범위·화면 역할·ROI가 맞을 때만 검증된 행동 경로를 재생합니다. 초기 조건이 맞지 않으면 이번 재생을 건너뛰고, 실행 중 또는 사후 검증이 실패하면 해당 경로를 차단한 뒤 LLM 탐색으로 복귀합니다.
- 성공률과 필드 정확성, Trace, Agent 판단, OCR, 토큰, 지연 시간과 비용을 나눠 측정하고 자동 검사·사람 감사·회귀 테스트의 역할을 분리합니다.
...
2. 수집에 성공하면 행동 전 화면, 클릭·입력, 행동 후 화면을 `Action Transition Trace`로 기록합니다.
3. 실행 그래프가 연속 행동을 의미 단위로 묶고 Critic이 검증한 노드만 `Validated Action-path`로 승격합니다.
4. 다음 실행에서 URL 범위·화면 역할·ROI pHash를 `Preconditions / Guardrails`로 검사하고, 조건이 맞을 때만 저장 행동을 재생합니다.
5. 초기 조건이 맞지 않으면 LLM 탐색으로 전환하고, 실행 중 또는 사후 효과 검증이 실패하면 남은 경로를 막은 뒤 `Safe Fallback`으로 복귀합니다.
...
| URL·화면 역할·ROI 조건 검사 | [_screen_precondition_reason](./agent/recipe/replay.py#L271-L284) · [_saved_target](./agent/recipe/replay.py#L305-L323) |
| FastAPI·SSE 요청 경계 | [chat_endpoint](./agent/web_server.py#L167-L199) |
| Trace·토큰·비용 집계 | [RunContext.snapshot](./agent/observability/run_context.py#L257-L303) |
...
표는 두 방식 모두 자동 수집 계약을 통과하고 사후 확인에서 상세 URL도 같았던 8쌍만
비교하며 별도 사람 필드 감사 표본은 아닙니다. 이 8개 경로를 한 번 검토·저장하는
...
실패하면 즉시 LLM 자율 탐색으로 돌아갑니다. 이후 양쪽 방식이 모두 자동 수집 계약을
통과하고 사후 확인에서 상세 URL도 같았던 8쌍에서 화면 판단 16.4%, OCR 22.4%, 토큰
13.2%가 줄었지만 추정 비용은 2.7% 늘었습니다. 따라서 성과를 전체 비용 절감이 아니라
**검증 가능한 반복 구간의 화면 판단 감소**로 한정했습니다.
