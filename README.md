...
- [x] Phase 1: Classic 시스템 베이스라인
  ...
  - [x] Qwen 모델 활용 LLM 정형화 및 SQLite 저장
  - [x] LLM 출력 JSON 모드 및 타입 정규화
  - [x] 사이트별 어댑터 패턴 및 URL 디스패처 classic/automation/sites/
  - [x] 5개 주요 사이트 안정화 원티드, 잡코리아, 사람인, 워크넷, 로켓펀치

- [ ] Phase 2: 비전 및 물리 제어 엔진 기반 에이전트 도구 구축
  - [ ] 1. 지표 및 에러 추적 세팅
    - [ ] sentry-sdk: 시스템 크래시 및 좌표 이탈 등 치명적 에러 캡처
    - [ ] structlog: 소요 시간 등 성능 벤치마크용 JSON 포맷 로깅
  - [ ] 2. 백그라운드 엔진 스크립트 ※ LLM 직접 호출 불가
    - [ ] Perception: mss 모듈 활용 전체 모니터 화면 캡처 → OmniParser 분석 → 마커 합성
    - [ ] Wait Stable: 무한 대기 버그를 막기 위해 픽셀 오차율 1퍼센트 이하 조건 및 최대 대기 시간 5초를 적용한 시각적 화면 안정화 대기
    - [ ] Security: OS keyring 기반 자격증명 관리 시스템
  - [ ] 3. 순수 파이썬 좌표 검증 테스트
    - [ ] LLM 연동 전 캡처 화면의 마커 좌표와 실제 마우스 클릭 좌표가 어긋나지 않는지 하드코딩으로 1차 확인
  - [ ] 4. 에이전트 상태 관리
    - [ ] 스크롤이나 클릭 동작의 성공 여부를 비교할 수 있도록 최근 2장의 전후 마커 이미지 보관
    - [ ] 파싱된 UI 요소 텍스트 및 Action History 정의
  - [ ] 5. LLM 전용 물리 행동 도구 ※ 에이전트의 유일한 인터페이스
    - [ ] click_marker: 마커 ID의 절대 좌표 계산 후 PyAutoGUI 물리적 클릭
    - [ ] type_in_marker: 한글 씹힘 현상 방지를 위해 pyperclip 모듈을 활용한 클립보드 복사 후 붙여넣기 물리 타이핑
    - [ ] scroll: OS 마우스 휠 스크롤
    - [ ] press_key: Enter, ESC 등 특수키 입력
    - [ ] finish_task: 수집 완료 시 데이터를 반환하며 루프 강제 종료
    - [ ] Action Wrapper: 도구 실행 시 성능 로깅 및 안정화 대기 로직 자동 주입

- [ ] Phase 3: LangGraph 지휘자 워크플로우 구성
  - [ ] 1. 노드 설계 및 관찰 계획 행동 루프
    - [ ] Perception Node: 시스템이 화면을 캡처하고 Qwen 비전 모델이 분석하여 요소들을 JSON 텍스트로 추출 후 상태 갱신
    - [ ] Reasoning Node: 토큰 비용 절감을 위해 Claude 지휘자는 이미지를 보지 않고 Qwen이 넘겨준 텍스트 정보만 읽어 다음 행동 도구 선택
    - [ ] Action Node: 도구 실행 및 시스템 안정화 후 다시 Perception Node로 회귀
  - [ ] 2. 모듈화 기반 서브 그래프 구축
    - [ ] 향후 Phase 6의 라우터 확장을 고려하여 구조를 유연하게 분리
  - [ ] 3. LangSmith 통합 트래킹
    - [ ] 노드 간 궤적, 소요 토큰 수, 프롬프트 입출력 모니터링 적용
  - [ ] 4. 단일 시나리오 E2E 통합 테스트
    - [ ] 바탕화면에서 브라우저 조작까지 검증하는 "원티드에서 데이터 분석가 신입 공고 모아줘" 명령 테스트
