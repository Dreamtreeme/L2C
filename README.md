...
화면-행동-결과 기록과 조건부 재생을 설계했습니다. [실험 설계](./docs/site_onboarding_benchmark_plan.md#codex-실행-조건)와
[원본 비교 결과](./docs/evidence/site_onboarding/comparison_report.json)에 조건과 수치를 보존했습니다.

이 비교에서는 Codex에 네 격리 환경의 구현·브라우저 실행·테스트·오류 수정을 위임했습니다.
개발자는 기준점, 방식별 금지사항, 평가 검색어와 품질 게이트를 직접 설계했습니다. Vision
실패 과정에서는 단순히 LLM 호출 한도를 늘리는 안을 폐기하고, 완료·상세 검토 시점을 상태
전이로 결정하도록 방향을 제시했습니다. Classic의 기술 개입은 0회, Vision은 인크루트 1회·
랠릿 9회로 기록됐습니다. 최종적으로 비교 결과를 해석해 위 설계 전환을 결정했습니다.
