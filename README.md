...
- [x] Phase 2: 비전 및 물리 제어 엔진 기반 에이전트 도구 구축
  ...
  - [x] 2. 백그라운드 엔진 스크립트 ※ LLM 직접 호출 불가
    - [ ] Perception: mss 모듈 활용 전체 모니터 화면 캡처 → OmniParser 분석 → 마커 합성 (진행 중)
...
- [ ] Phase 3.5: 로컬 VLM 최적화 도전 (Ollama -> Hugging Face 직접 구동)
  - [ ] 1. PyTorch + CUDA 기반 Hugging Face Transformers 연동 및 추론 모듈 구현
  - [ ] 2. Windows 환경 내 Flash Attention 2 수동 컴파일 및 GPU 가속 최적화
  - [ ] 3. max_pixels 설정을 통한 고밀도 해상도 보존 및 다이내믹 시각 토큰 압축 적용
