...
순서와 화면 조건을 새로 만들지 않습니다. 재생 시 현재 화면의 ROI pHash를 저장된
서명과 비교하고, 일치하면 저장된 대상 중심 좌표 비율로 물리 입력 위치를 복원합니다.
...
관측 계약은 성공적으로 재생한 규칙이 대체한 원본 판단 수를
`reflex_reasoning_call_reduction`으로 기록합니다. 실행 조건과 원본 집계값은
...
|---|---|---|---|
| 첫 사이트 준비 | selector와 실행 순서 구현 | 목표와 접속 정보 제공 | 선언형 프로필 작성과 자율탐색 |
| 화면 판단 | DOM 조건 | 매 단계 모델 판단 | 저장 경험이 없을 때 모델 판단 |
| 반복 행동 | 작성한 코드 실행 | 다시 모델 판단 | 관찰된 성공 전이 재생 |
| UI 변경 시 실행 | 실패한 selector 수정 | 현재 화면 재추론 | ROI 불일치 시 자율탐색 복귀 |

Classic 기준선은 원티드, 잡코리아, 로켓펀치, 인크루트와 랠릿의 DOM 어댑터를 갖고 있습니다.
Realtime/Vision 경로는 사이트별 selector 없이 클릭, 입력, 스크롤, 키 입력과 브라우저
이동 도구를 공유합니다.

인크루트와 랠릿을 T0 기준선에 새로 추가하는 동일 과업으로 Classic과 Vision을
비교했습니다. 두 방식 모두 세 미사용 검색어의 수집 계약과 12개 실행의 결과 의미
검토를 통과했습니다.

| 사이트 | Classic 적용시간 | Vision 적용시간 | Classic/Vision 사이트 전용 코드 | Classic/Vision 실행 중앙값 |
|---|---:|---:|---:|---:|
| 인크루트 | 22분 31초 | 52분 16초 | 158줄 / 30줄 | 14.87초 / 59.74초 |
| 랠릿 | 23분 3초 | 58분 6초 | 114줄 / 81줄 | 7.09초 / 45.10초 |

Vision은 사이트별 결합 코드를 인크루트 81%, 랠릿 29% 줄였습니다. 반면 두 사이트 모두
적용시간은 Classic보다 길었고, Vision 공통 런타임 수정도 각각 153줄과 134줄
필요했습니다. 이 표본에서 확인된 장점은 **사이트별 코드를 선언형 프로필로 줄인 것**이며,
전체 개발 공수와 반복 실행 비용 절감은 확인되지 않았습니다. 원시 세션과 계산 결과는
[`docs/evidence/site_onboarding/comparison_report.json`](./docs/evidence/site_onboarding/comparison_report.json)에
보존했습니다.
  ...
  → Investigation LangGraph
      ...
      → WorkerExecutionService
          → Vision Worker LangGraph
              ...
              → 공고 검토 → 계속 읽기/완료/자료 부족/잘못된 대상
      → 검토 완료 공고 전달 → 공고 저장 → DB 재검사
      → job_id 근거 답변
...
- `agent/application/`: 수집 실행, 공고 검토, DB 조회·저장 서비스
