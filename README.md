...
- [x] Phase 6: 수집 데이터 전처리 및 DB 적재 신뢰성 강화
  - [x] 1. 텍스트 전처리 및 노이즈 필터링 파이프라인 구축 (`preprocessor.py`)
    - [x] OCR 텍스트 내 불필요 개행, 특수 기호, 마커 잔영(`[id]`) 제거
    - [x] 줄글 형태 리스트 항목의 원자적 분할(`list[str]`) 및 정규화
  - [x] 2. 스키마 기획 및 유효성 검증 (`jd_schema.py`)
    - [x] `JobPosting` Pydantic 모델을 통한 타입 검증 및 데이터 형식 일치화
  - [x] 3. SQLite DB 영속화 연동
    - [x] URL 고유 키 기준 `UPSERT` 처리로 중복 적재 방지 및 데이터 갱신 로직 구현
  - [x] 4. E2E 데이터 파이프라인 정합성 최종 검증 (`test_db_persistence.py`)
    - [x] 실제 비전 에이전트로 긁은 수집 데이터에 대해 전처리-DB 연동 후 적재 누락 체크 자동화

- [ ] Phase 7: 하이브리드 RAG 데이터 Q&A 에이전트 확장
