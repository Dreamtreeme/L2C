...
| `worker_reasoning_prompt.py` | 화면·수집·전환 정보를 추론 메시지로 조립 |
...
| 모듈 | 책임 |
|---|---|
| `investigation_context.py` | 책임별 조사 상태, 모델 묶음, 공통 요청 문맥 |
...
| `investigation_workflow.py` | 주입된 노드 연결, 체크포인트 중단·재개, 실행 진입 |
