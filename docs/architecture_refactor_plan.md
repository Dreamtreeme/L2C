...
- 전환 판정: 이전 행동의 화면 변화와 전환 계약만 평가한다.
- 행동 선택: 큐, 상세 정책, Reflex, LLM 순서로 하나의 `ActionRequest`를 만든다.
- 행동 실행: 화면 변화가 예상되는 첫 원자 행동까지만 실행한다.
...
- 빈 격리 DB의 원티드 2건 cold E2E는 67.61초에 2건 저장·정상 종료했다. 기존 운영 DB 실행은 중복 공고를 여러 번 건너뛰며 그래프 예산을 소진해 1건 부분 완료했으므로 별도 수집 정책 문제로 구분한다.
...
- 실행기와 추론 문맥 분리 후 Python 3.13 전체 단위 테스트 218건을 통과했다.
- 모든 수치는 텍스트 로그 재계산이 아니라 각 실행의 `.summary.json`을 기준으로 확인했다.

| 시나리오 | 실행시간 | 품질 | 주요 관측 |
|---|---:|---|---|
| 원티드 cold, iOS 2건 | 63.30초 | 2건 저장, 통과 | 카드 큐 재생 1회, OCR timeout 0회 |
| 사람인 cold, 머신러닝 1건 | 39.14초 | 1건 저장, 통과 | OCR timeout 0회 |
| 로켓펀치 cold, 백엔드 1건 | 43.91초 | 1건 저장, 통과 | SPA 측면 상세 URL 증거 보완 후 재검증 |
| 고용24 cold, 데이터 분석 1건 | 37.33초 | 1건 저장, 통과 | 후보 저장 후 Critic 승격 성공 |
| 고용24 clean warm, 동일 요청 | 43.33초 | 1건 저장, 통과 | Reflex 1회, 후속 전략 1회, 성능 비교 가능 |
| 원티드 운영 DB, iOS 2건 | 89.94초 | 새 1건·기존 10건 근거로 통과 | 재귀 한도 도달, 활성 레시피 없음 |
...
고용24 warm은 Reflex 기능과 품질은 통과했지만 cold보다 빨라지지는 않았다. 따라서 레시피 적중은 추론 경로를 줄이는 기능 증거로만 사용하며, 한 번의 실행시간 개선을 일반화하지 않는다.

운영 DB 원티드는 기존 공고를 품질 충족 근거로 계산하면서도 작업자 그래프가 새 공고 탐색을 계속해 재귀 한도에 도달했다. 이는 구조 리팩터링 회귀가 아니라 중복이 많은 목록의 종료·예산 정책 한계로 남긴다.
...
- 목록 복귀 후 결과 카드 큐 재생
- 화면 변경을 확인하는 전환 계약
- 자율탐색 결과를 후보로 저장한 뒤 별도 Critic이 승격하는 흐름
