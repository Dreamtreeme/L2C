...
> [!warning] 역사 자료
> 이 문서는 2026-07-24부터 2026-08-14까지 서로 다른 코드·화면·모델 조건에서
> 관찰한 실행 기록을 보존합니다. 현재 성능 기준선으로 사용하지 않습니다. 현재 재현
> 절차는 [제품 데모 가이드](../product_demo.md), 최신 집계는
> [현재 버전 검증 기록](../evidence/l2c_metrics_20260822.md)을 우선합니다.

> [!info] 검증 범위
> 이 문서는 로컬 Chat UI부터 조사 LangGraph, 비전 작업자, SQLite 근거 답변까지 실제 연결된 제품 경로를 설명합니다. 특정 실행의 성공을 모든 사이트의 성공률로 확대 해석하지 않습니다.

> [!warning] 경험 기반 탐색 수치 해석
> 아래 자율탐색·경험 기반 탐색 표의 실행시간, 토큰과 비용은 당시 실행의 관찰값입니다. 자율탐색 당시 화면과 모델 판단을 다시 만들 수 없으므로 두 행의 차이를 경험 규칙의 절감량이나 손익분기점으로 계산하지 않습니다.
...
테스트 DB는 91건에서 92건으로 증가했습니다. 해당 수집 실행에서 확인한 문서
ID는 `10`, `11`, `94`였고, 수집 후 의미 검증은 이 범위 안에서 백패커의
`[텀블벅] iOS 개발자 (1-3년)` 공고 `job_id:10`을 최종 근거로 선택했습니다.
따라서 새 행뿐 아니라 기존 중복 공고를 다시 관찰한 경우에도 해당 실행에서
...
첫 사람인 경험 기반 탐색은 활성 ROI 레시피가 없어 자율 탐색으로 폴백했습니다.
후보를 Critic으로 승격한 다음 실행에서도 Reflex는 0회였습니다. 레시피가 요구한
입력 이름은 `search_keyword`였지만 작업자 상태가 `query`를 전달해 모든 후보가
화면 비교 전에 `missing_required_inputs`로 탈락한 것이 원인이었습니다.

작업자 입력 계약을 `search_keyword`로 통일한 재검증에서는 검색어 입력과 검색
버튼 클릭이 Reflex로 재생됐습니다. 실행시간은 41.12초, 화면 추론은 1회,
Reflex는 2회, 토큰은 14,799, 추정 비용은 $0.0050이었습니다. 검색 결과의 공고가
직전 실행에서 저장한 DB ID 150과 같아 상세 화면을 다시 열지 않고 종료했으며,
이 기록은 반복 행동과 중복 조기종료 검증에 사용합니다. 신규 상세 수집의 성능
비교에는 포함하지 않습니다.
...
모든 행은 목표 수와 DB 저장 품질을 통과했습니다. 실행별 총시간은 서로 달랐지만
상세 탐색 경로와 모델 지연이 섞여 있어 경험 규칙의 효과로 분리할 수 없습니다.
경험 규칙의 성과는 대체한 원본 판단 수에서 재생 해석기 호출을 뺀 순감소량으로 판정합니다.
...
여섯 실행 모두 목표 수와 저장 품질을 통과했습니다. 총 실행시간 차이는 상세 탐색과
외부 지연을 포함하므로 경험 규칙 성과로 사용하지 않습니다. 성공한 재생 단계가
추론 호출 순감소량과 폴백 여부를 직접 집계합니다.
...
기존 summary 150개를 커밋과 설정 기준으로 분류했고, 그중
...
원티드 두 행에서는 실행시간, 추론과 토큰이 낮게 관찰됐습니다. 이 차이는 재현할 수
있는 짝 실험이 아니므로 경험 규칙의 절감률이나 Critic 비용 회수 횟수로 사용하지
않습니다.
...
단계가 전체 경로 비용을 늘릴 수 있다는 당시 확인한 한계**로 해석합니다.

### 2026-08-12 상태·생명주기 회귀

깨끗한 커밋 `8c36be3`에서 원티드와 사람인의 자율·경험 기반 실행을 각각 한 번씩 기록했습니다.

| 사이트·모드 | 실행시간 | 추론 | OCR | Reflex | 완료 경로 | 토큰 | 추정 비용 |
|---|---:|---:|---:|---:|---:|---:|---:|
| 원티드 자율 탐색 | 61.36초 | 10회 | 8회 | 0회 | 0개 | 65,279 | $0.0379 |
| 원티드 경험 기반 탐색 | 56.26초 | 8회 | 8회 | 3회 | 2개 | 55,439 | $0.0327 |
| 사람인 자율 탐색 | 76.12초 | 13회 | 11회 | 0회 | 0개 | 91,981 | $0.0520 |
| 사람인 경험 기반 탐색 | 74.07초 | 10회 | 11회 | 3회 | 2개 | 74,066 | $0.0382 |

네 실행 모두 2건 수집·저장과 품질 게이트를 통과했습니다. Critic 승격에는
원티드 9.57초·`$0.0148`, 사람인 10.18초·`$0.0132`가 별도로 들었습니다.
이 표는 실행 기록과 경로 회귀 통과 근거이며 비용 손익분기점은 계산하지 않습니다.
...
없으므로 이 차이를 실행시간 또는 추론 3회 절감으로 판정하지 않습니다. 해당
기록에서는 성공적으로 재생한 규칙이 대체한 원본 판단 수를
`reflex_reasoning_call_reduction`으로 기록합니다. 실행 조건과 원본 집계값은
[고정 대상 증거](../evidence/saramin_target_contract_pair_20260814.json)에 보존했습니다.

### 장기 실행 자원 재사용

같은 FastAPI 수명에서 원티드 iOS와 백엔드 개발자 수집 요청을 연속
실행했습니다.
...
| 실행시간 | 69.55초 | 83.88초 |
| OCR 시작 횟수 | 1회 | 0회 |
| OCR 작업자 PID | 19472 | 19472 |
| OCR 요청 | 3회 | 9회 |
| 추론 | 4회 | 9회 |
| 토큰 | 47,237 | 90,695 |
| 추정 비용 | $0.0753 | $0.1098 |

첫 요청은 기존 DB 문서 `147`, 두 번째 요청은 새 문서 `151`을 수집 결과와
최종 답변에서 같은 ID로 인용했습니다. 두 요청 모두 제품 계약을 통과했고,
각 요청이 끝난 뒤
브라우저 창은 닫혔고, OCR 작업자와 reasoning 모델 캐시는 같은 백엔드
프로세스에서 재사용됐습니다. 이 검증은 같은 사이트의 요청 2개로 자원 수명과
인용 연결을 확인한 것이며 사이트 간 성공률 표본은 아닙니다.
...
### 2026-08-12 Reflex Recipe 계약 전환 검증

원티드 `iOS 개발자 1건`을 격리 DB에서
`자율탐색 → Critic 승격 → 경험 기반 탐색` 순서로 실행했습니다.
...
두 실행 모두 서로 다른 공고 1건을 저장했고 품질 계약을 통과했습니다. 경험 기반
탐색은 홈 검색 열기, 검색어 입력, 직무 결과 선택과 포지션 탭 선택으로 이어지는
4단계 경로를 중간 실패 없이 완료했습니다.
