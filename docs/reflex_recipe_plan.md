...
-> 백엔드 승격 작업자가 후보를 선점
-> 화면 변화 여부와 관계없이 원본 실행 이벤트 전체를 그래프 구성 모델에 전달
-> 모델이 하위 목적 노드와 next/branch/recovery/feedback 간선 구성
-> Critic이 완성된 실행 그래프를 보고 의미 노드 유지/제거 판정
-> 선택된 노드와 그 안의 원본 화면 전이를 active_recipe로 저장
...
활성 레시피는 한 성공 실행에서 선택된 의미 노드를 `ExperienceRule` 하나로 저장한다.
`ExperienceRuleNode`는 목적과 원본 이벤트 범위, 소속 물리 단계 ID를 보존한다.
각 `ExperienceRuleStep`은 하나의 원본 화면 전이를 `before + actions + expected_effect`로
...
경우에는 두 행동이 처음부터 같은 전이로 기록된다. 화면 변화가 작거나 없는 행동도
원본 로그와 실행 그래프에 남긴다. 그래프 구성 모델이 후속 행동과의 목적·인과관계를
표현하고 Critic이 전체 그래프를 본 뒤 노드 단위 유지 여부를 판단한다. 규칙 생성기는
노드를 다시 합치거나 나누지 않으며, 노드 내부 전이도 서로 합치지 않는다. 중간 화면을
봐야 다음 대상을 찾을 수 있기 때문이다.
...
코드는 화면 pHash나 전환 상태를 근거로 Critic 입력에서 행동을 선제 제거하지 않는다.
그래프 구성과 가지치기 결과가 컴파일 계약을 만족하지 못하면 일부 행동을 임의로
연결하지 않고 후보 전체를 거부한다.
...
자율탐색은 실제 도구 계약, 판단 이유와 화면 결과를 원본 이벤트로 기록한다. 그래프
구성 모델이 모든 이벤트를 목적별 노드와 인과 간선으로 정리한 뒤 Critic이 잘못된
대상, 무효 행동과 폐기된 복구 경로에 해당하는 노드를 제거한다. 한 노드에 정상 경로와
실패 경로가 섞였으면 전이를 임의로 분리하지 않고 후보 전체를 거부한다. 두 모델 모두
실행 내용을 수정하지 않는다.
...
행동은 서로 다른 요청과 전이로 기록한다. 모든 원본 이벤트를 실행 그래프에 포함한
뒤 Critic이 전체 문맥에서 유지 여부를 판정한다.
...
현재 집계는 [현재 버전 검증 기록](evidence/l2c_metrics_20260822.md)을 기준으로 한다.
2026-08-12 단일 실행의 계약 전환 과정은
[제품 회귀 실행 기록](history/product_regression_runs.md#2026-08-12-reflex-recipe-계약-전환-검증)에 보존한다.
