...
> 아래 자율탐색·경험 기반 탐색 표의 실행시간, 토큰과 비용은 당시 실행의 관찰값입니다. 자율탐색 당시 화면과 모델 판단을 다시 만들 수 없으므로 두 행의 차이를 경험 규칙의 절감량이나 손익분기점으로 계산하지 않습니다. 현재 성과 지표는 원본 판단 대체 수에서 재생 해석기 호출을 뺀 `reflex_reasoning_call_reduction`, 경험 경로 완료율, 폴백과 최종 수집 품질입니다.
...
단계가 전체 경로 비용을 늘릴 수 있다는 현재 한계**로 해석합니다.
...
- 경험 규칙의 추론 호출 순감소량과 경로 완료율의 다중 사이트 표본
- 실제 사용자 과업의 답변 만족도
