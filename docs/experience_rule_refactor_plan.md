...
현재 경험 기반 탐색은 자율탐색의 성공 경로를 기록하지만, 원본 관찰과 재사용 규칙이 같은 계약에 들어 있다.
...
- Critic은 기록된 행동 묶음을 유지하거나 제거한다. 행동을 새로 만들거나 수정하지 않는다.
...
`ExperienceRule`은 정제된 경로에서 LLM이 추출한 재사용 의미를 저장한다.
...
- [x] Critic의 keep/drop 출력과 권한 유지
- [x] Critic이 남긴 경로를 입력으로 받는 경험 규칙 생성 서비스 추가
- [x] 규칙 생성 LLM은 원본 행동 ID, 적용 조건, 대상 역할, 기대 효과만 출력
- [x] 원본에 없는 행동·입력·순서를 생성하면 구조 검증에서 거절
- [x] 연속된 입력·제출 전이는 중간 판단 없이 한 규칙 단계로 묶을 수 있게 출처 ID 목록으로 보존
- [x] Gemini 3.6 Flash 구조화 출력을 사용하고 호출 지표 기록

### 4. 경험 적용과 실행

- [x] 현재 화면에서 규칙의 빠른 적용 가능 여부 검사
- [x] 단일 대상이 확정되지 않으면 Gemini 3.6 Flash에 적용 또는 거절만 요청
...
- [x] 독립 스크롤 영역은 한 번 선택한 물리 지점과 검증 ROI를 실행 중 유지
...
- 현재 코드와 `v25`, `v29`, `v30` 실행 결과를 기준선으로 기록했다.
...
- Critic은 원본 전이의 keep/drop만 결정하고, `experience_rule_compiler.py`가 별도 Gemini 3.6 Flash 구조화 호출로 의미 규칙을 생성한다.
- 생성기는 원본 전이와 행동 ID를 빠짐없이 순서대로 참조해야 하며, 새 행동·잘못된 입력 슬롯·근거 없는 효과를 출력하면 저장하지 않는다.
- 입력과 제출처럼 함께 실행해야 효과가 생기는 인접 전이는 원본 ID를 모두 보존한 한 규칙 단계로 묶을 수 있다.
- 정확 ROI·현재 마커가 일치하면 모델 없이 실행하고, 모호하면 `experience_rule_resolver.py`가 적용 또는 거절과 현재 허용 마커 ID만 반환한다.
- URL 이동, 페이지 역할 변화, 전체 화면 변화와 대상 영역 변화를 `ExpectedEffect`로 분리했다.
- 독립 스크롤 패널은 첫 물리 지점을 세션에 저장하고 이후 OCR ID 대신 같은 정규화 좌표를 재사용한다.
...
- `marked_screen_20260815_172324_411221.png`에는 같은 `풀스택 개발자` 제목이 왼쪽 목록과 오른쪽 상세 패널에 함께 있다. 저장된 오른쪽 bbox와 두 현재 마커를 `match_exact_target`에 넣었을 때 오른쪽 마커 `46`을 선택했다.
...
- 필수 필드 충족 여부는 수집 후 `collection_postprocessing`에서 한 번 판정한다.
...
- 공고 `2/2`를 저장했으며 실행시간은 `75.846초`, Reflex hit `1회`, LLM `14회`, `89,434토큰`, `$0.0655058`, OCR `11회`였다.
...
- 사전조건은 활성 경험 규칙 `1개`를 확인했다. 현재 계약에서는 이를 `replay_ready=true`로 기록한다.
