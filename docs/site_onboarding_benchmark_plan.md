...
네 세션은 같은 `T0`, 수집 계약과 시간 제한을 사용했다. 두 사이트의 Classic과 Vision
모두 최종 자동 품질 계약을 통과했다. 방식 라벨을 제거하고 순서를 섞은 12개 실행의
24개 공고도 제목, 회사, URL과 구조화 본문 근거 검토를 통과했다. 이 검토는 독립 사용자
평가가 아니라 단일 검토자 판정이다.

| 사이트 | 방식 | 적용시간 | 사이트 전용 코드 | 공통 런타임 코드 | 실행 중앙값 | 최종 3회 토큰 | 최종 3회 비용 |
|---|---|---:|---:|---:|---:|---:|---:|
| 인크루트 | Classic | 1,350.762초 | 158줄 | 16줄 | 14.872초 | 12,607 | $0.011819 |
| 인크루트 | Vision | 3,135.598초 | 30줄 | 153줄 | 59.743초 | 164,134 | $0.097931 |
| 랠릿 | Classic | 1,382.662초 | 114줄 | 16줄 | 7.094초 | 8,467 | $0.007693 |
| 랠릿 | Vision | 3,485.864초 | 81줄 | 134줄 | 45.099초 | 122,451 | $0.062013 |

판정은 `혼합`이다. Vision은 사이트 전용 코드를 두 사이트 모두에서 줄였지만 적용시간은
각각 Classic의 2.32배와 2.52배였다. 반복 실행 중앙값과 토큰·비용도 Classic이 낮았다.
이번 표본은 Vision이 사이트 결합 지점을 프로필로 압축한다는 점을 보여주며, 전체 개발
공수나 실행 비용이 더 낮다는 주장은 지지하지 않는다.

구조화 결과는 `docs/evidence/site_onboarding/comparison_manifest.json`,
`comparison_report.json`, `blind_quality_report.json`에 보존한다.
