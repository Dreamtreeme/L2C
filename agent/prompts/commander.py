...
- **[채용 공고 본문 수집 지침]**: 채용 공고 상세 페이지에 진입하면 먼저 '상세 정보 더 보기' 또는 '더보기' 버튼 마커를 클릭하여 본문을 완전히 펼쳐야 합니다. 만약 화면 내에 자격요건이나 우대사항 등의 본문이 잘려서 보이지 않는다면, `scroll` 도구(방향: 'down', clicks: 800)를 실행하여 아래로 스크롤한 후 본문을 완전히 판독하십시오. 최종 수집을 마쳤으면 `finish_task`를 호출해 수집된 정보(회사명, 직무명, 주요업무, 자격요건, 우대사항, 혜택 등)를 JSON 형태로 담아 반환하십시오.
"""
