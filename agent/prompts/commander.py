...
- 예 2: 상세 페이지에서 새로운 정보를 발견한 경우: `update_extracted_info`만 호출하고 `page_role="job_detail"`, `detail_complete`를 함께 표시하십시오. 상세가 아직 부족하면 executor가 저해상도 화면+OCR로 페이지 정책을 판단한 뒤 숨겨진 본문 펼침, 스크롤, 보류 중 하나를 실행합니다.
...
- **[다중 공고 누적 수집을 위한 데이터 구조]**:
  ...
  - `detail_complete=false`일 때는 `scroll`을 직접 체이닝하지 마십시오. executor가 별도 페이지 정책 판단으로 숨겨진 본문 펼침, 아래 스크롤, 보류 중 하나를 선택합니다.
...
- **[하향 스크롤 수집 및 중복 방지 (매우 중요)]**:
  ...
  - `detail_complete=false` 이후의 숨겨진 본문 여부와 스크롤 필요성은 저해상도 화면+OCR 정책 판단이 한 번 더 확인합니다. 당신은 새로 캡처된 화면에서 추가 정보와 완료 여부를 판단하십시오.
- **[탐색 대 종료 자가판단]**:
