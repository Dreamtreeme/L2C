...
- **[탐색 대 종료 자가판단 (중요)]**:
  - 채용 공고 상세 페이지에 들어갔을 때, 회사명, 직무명, 주요업무, 자격요건, 우대사항, 혜택/복지 등 모든 핵심 정보가 화면 상에 완전히 식별되었는지 스스로 검증하십시오.
  - 만약 '상세 정보 더 보기' 혹은 '더보기' 버튼 마커가 존재한다면 반드시 클릭하여 본문을 펼치십시오.
  - **[더보기 클릭 후 필수 동작]**: '더보기' 버튼을 클릭한 직후에는 늘어난 본문 내용으로 인해 혜택/복지 등 하단 정보가 화면 아래로 밀려나게 됩니다. 따라서 더보기를 클릭한 후에는 성급히 finish_task를 호출하지 말고, **반드시 최소 1회 이상 `scroll` 도구(방향: 'down', clicks: 800)를 사용하여 아래로 화면을 밀어 하단부 영역까지 완전히 확인**하십시오.
  - 화면을 끝까지 내리고 모든 본문 필드를 판독 완료했다고 판단될 때만 `finish_task`를 호출하고, 수집한 정보를 JSON 구조로 반환하십시오.
"""
