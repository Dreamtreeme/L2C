...
- **[탐색 대 종료 자가판단 (중요)]**:
  ...
  - 만약 '상세 정보 더 보기' 혹은 '더보기' 버튼 마커가 존재한다면 반드시 먼저 클릭하여 본문을 펼치십시오.
  - 본문을 펼친 후에도 화면 내에 주요업무, 자격요건, 우대사항, 혜택 중 일부 항목이 완전히 표시되지 않았거나 하단부 영역이 뷰포트(화면)에 보이지 않는다면, `finish_task`를 성급히 호출하지 말고 **반드시 `scroll` 도구(방향: 'down', clicks: 800)를 통해 아래로 스크롤하여 탐색을 연장**해야 합니다.
  - 모든 본문 필드를 화면상에서 끝까지 읽어 판독을 완전히 마쳤다고 판단될 때만 `finish_task`를 호출하고, 수집한 정보를 JSON 구조로 반환하십시오.
"""
