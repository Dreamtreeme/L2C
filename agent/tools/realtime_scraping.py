...
def realtime_scraping(company: str = None, tech_stack: str = None) -> str:
    ...
    try:
        ...
        goal = (
            f"원티드(https://www.wanted.co.kr)에서 '{search_keyword}'를 검색하세요. "
            f"검색 결과 화면에 도달하면 먼저 화면에 노출된 채용 공고 수와 카드 목록을 확인하고, "
            f"수집할 공고별 순회 계획을 세운 뒤 상세 페이지를 하나씩 방문하세요. "
            f"상세 페이지에서 회사명, 직무명, 주요업무, 자격요건, 우대사항, 혜택 등의 정보를 수집한 뒤 "
            f"목록으로 돌아와 이미 클릭한 카드는 제외하고 다음 공고를 선택하세요."
        )
