...
def test_reflex_path_observation_tracks_lifecycle_events() -> None:
    ...
    assert completed["reflex_resolver_reasoning_call_count"] == 1
    assert completed["reflex_reasoning_call_reduction"] == 1
...
def test_reflex_path_summary_separates_step_hits_from_path_completion() -> None:
    steps = [
        ...
        },
        {
            ...
            "reflex_resolver_reasoning_call_count": 0,
    ...
    assert summary["reflex_resolver_reasoning_call_count"] == 1
    assert summary["reflex_reasoning_call_reduction"] == 2
...
def test_reflex_resolver_calls_on_miss_are_negative_reduction() -> None:
    observation = reflex_selection_observation(
        {
            "replay": {
                "reflex_trace": {
                    "hit": False,
                    "reason": "no_rule_applied",
                    "resolver_reasoning_call_count": 2,
                }
            }
        }
    )

    assert observation["reflex_resolver_reasoning_call_count"] == 2
    assert observation["reflex_reasoning_call_reduction"] == -2


def test_reflex_pending_transition_does_not_count_resolver_twice() -> None:
    ...
    assert "reflex_resolver_reasoning_call_count" not in observation
