...
def _metric_steps(final_payload: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [item for item in metrics.get("steps", []) if isinstance(item, dict)]
...
def _reuse_quality(runs: list[dict[str, Any]]) -> dict[str, Any]:
    snapshots = [dict(run.get("resource_snapshot") or {}) for run in runs]
    ...
    return {
        "request_quality_passed": bool(runs)
        and all(bool((run.get("quality") or {}).get("passed")) for run in runs),
        "same_ocr_worker_pid": (
            len(pids) >= 2
        ...
        "browser_closed_after_each_request": bool(snapshots)
        and all(not snapshot.get("browser_window_bound") for snapshot in snapshots),
...
def main() -> int:
    ...
    parser.add_argument("--conversation-prefix", default="product-e2e")
    ...
    if len(runs) > 1:
        ...
        quality["passed"] = bool(not error and all(quality.values()))
    else:
        request_passed = bool(
            runs and (runs[0].get("quality") or {}).get("passed")
        )
        quality = {
            "request_quality_passed": request_passed,
            "passed": bool(not error and request_passed),
        }
    summary = {
        "schema_version": 1,
