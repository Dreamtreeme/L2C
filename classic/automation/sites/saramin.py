...
class SaraminAdapter(SiteAdapter):
    ...
    def extract(self, page) -> dict:
        ...
        if not full_text.strip():
            logger.debug(
                "[saramin] 메인 페이지에서 본문을 찾지 못함. iframe(#iframe_content_0) 탐색 시작..."
            )
            try:
                frame_loc = page.frame_locator("#iframe_content_0")
                t = (
                    frame_loc.locator(".jv_cont.jv_detail")
                ...
                logger.debug(
                    f"[saramin] frame_locator 탐색 실패 (CORS 문제일 수 있음): {e}"
                )
        ...
        if not full_text.strip():
            ...
            if m:
                ...
                logger.info(
                    f"[saramin] iframe 접근 완전 실패. HTTP GET으로 우회 추출: {detail_url}"
                )
                try:
                    ...
                    if text:
                        full_text = text
                        logger.info(
                            "[saramin] HTTP GET 우회 상세 페이지에서 본문 추출 완료"
                        )
                except Exception as e:
                    logger.warning(
                        f"[saramin] HTTP GET 우회 상세 페이지 추출 실패: {e}"
                    )
