...
## 24. 검색어 입력만 반복 재생해 경험 기반 탐색이 느려진 문제
...
- 같은 화면 역할과 URL에서 연속 성공한 `type_in_marker -> click_marker`를 하나의
  `transition_action_set`으로 승격한다.
- 첫 캡처에서는 입력 ROI만 확인하고 현재 단계 하나를 실행한다.
- 중간 전환 성공 뒤 다음 캡처에서 제출 버튼 ROI를 확인하고 세트의 다음 단계를 이어간다.
- 세트 진행 중에는 다른 후속 전략이 끼어들지 못하게 한다.
...
제출 ROI가 불일치했다. 그 결과 LLM이 검색어를 먼저 입력한 다음 행동 세트가
처음부터 재생되어 검색어를 두 번 입력했다.

이를 다음 단계 번호를 기억하는 실행 방식으로 교체했다.

- 첫 화면에서는 입력 ROI만 검증하고 `1/2` 입력을 실행한다.
- 새 캡처와 OCR에서 입력 성공과 제출 ROI를 확인한다.
- LLM 호출 없이 `2/2` 제출 클릭을 실행한다.
- 마지막 전환 성공 시 세트를 정리하고, 중간 실패 시 세트를 폐기한 뒤 자율추론으로 폴백한다.
