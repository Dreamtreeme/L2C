...
### [현재 상태]
현재 pHash 기반 replay 검증은 테스트 중이다. 구현 방향은 절대좌표를 무조건 재사용하는 것이 아니라, `pHash + OCR anchor + target bbox 비율 + 현재 OCR marker`를 함께 검증한 뒤 통과한 경우에만 행동을 재생하는 방식이다.

따라서 이번 단계의 검증 포인트는 다음이다.

- 반복탐색에서 실제 reasoning 호출 횟수가 줄어드는가
- 자율탐색과 동일한 수집 품질이 유지되는가
- 로딩 화면, 팝업, 광고, 검색 결과 변화에서 잘못된 replay가 발생하지 않는가
- pHash 불일치 또는 OCR 타깃 불일치 시 reasoning 폴백이 정상 작동하는가
