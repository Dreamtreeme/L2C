...
### [실제 원인]
외부에서 PaddleOCR에 전달하는 이미지 크기가 핵심 변수였다. 원티드 E2E 화면은 브라우저 영역 crop 뒤 약 `1960x1939`이며, 기존 기본값은 최대 변 1280이었다. 텍스트가 밀집한 화면의 큰 recognition crop들이 GPU 경로의 비용을 급격히 키웠다.
...
1152는 1024와 지연 차이가 거의 없으면서 1280 결과와의 텍스트 줄 일치율과 검출 상자 수가 더 가까웠다. 대표 밀집 화면의 검출 상자 수도 `148→152`, `218→226`, `159→161`로 본문 양이 유지됐다.
...
### [결론]
worker 재시작이 빨라 보였던 것은 원인을 제거한 것이 아니라 이미 느려진 요청을 끊고 GPU 상태를 다시 준비한 결과였다. 재현 실험에서 실제 원인은 worker 사용 횟수가 아니라 큰 OCR 입력과 밀집 텍스트의 결합이었다. 입력 최대 변을 1152로 제한하자 worker를 계속 재사용해도 65회 동안 지연 폭증이 재현되지 않았다.
