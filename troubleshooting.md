...
- 실행 전 입력창과 제출 버튼 ROI를 모두 확인한다.
- 두 물리 행동은 각각 기록하되 화면 전환은 마지막 제출 클릭 뒤 한 번만 판정한다.
- LLM과 다른 정책의 복수 행동 요청은 계속 거절한다.

### [검증]

사람인 `AI 엔지니어 공고 1개` 회귀에서 행동 세트가 한 번 적중했고 입력과 검색
클릭을 0.29초에 연속 실행했다. 전환 결과는 즉시 `ready`였으며 반복 입력과 pHash
대기 6회가 사라졌다. 경험 기반 탐색 실행시간은 직전 48.16초에서 42.23초로
줄었고 공고 1건 저장과 품질 계약을 모두 통과했다.
