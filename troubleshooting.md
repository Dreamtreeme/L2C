...
- 모든 프로젝트 가상환경을 PaddlePaddle GPU 3.0.0과 CUDA 11.8 조합으로 통일했다.
...
### [결론]
AI의 라이브러리 버전 추천을 검증 없이 채택하고, 나중에 바뀐 의존성 선언을 기존 가상환경에 반영하지 않은 것이 문제의 출발점이었다. worker 재시작과 1152 리사이즈는 현상을 일시적으로 완화했지만 근본 해결은 아니었다. PaddlePaddle GPU 3.0.0으로 통일한 뒤 기본 `.venv`에서 50회 연속 OCR이 모두 성공했고 p95 0.761초를 유지했다. 실제 사람인 1건 E2E도 OCR 10회, 최대 1.55초, timeout 0회로 완료됐다.
